> [!IMPORTANT]
> Hola SCP.Net versión utilizado en el proyecto debe coincidir con hello Storm presente en el clúster de HDInsight. Usar hello después toodetermine tabla qué versión debe utilizar:
> 
> | Versión de HDInsight | Versión de Apache Storm | Versión de SCP.NET | Versión de Mono predeterminada |
> |:---:|:---:|:---:|:---:|
> | 3.3 |0.10.0.x |0.10.0.x (solo clústeres basados en Windows) | N/D |
> | 3.4 |0.10.0.x |0.10.0.x | 3.2.8 |
> | 3,5 |1.0.2.x |1.0.0.x | 4.2.1 |
> | 3.6 |1.1.0.# | 1.0.0.x | 4.2.8 |
> 
> Para más información sobre los componentes que se suministran con HDInsight, consulte [Versiones de los componentes de HDInsight](../articles/hdinsight/hdinsight-component-versioning.md).


