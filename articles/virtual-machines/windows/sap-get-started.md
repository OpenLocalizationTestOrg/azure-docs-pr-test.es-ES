...
Si elige Microsoft Azure como su asociado de listo en la nube SAP, será tooreliably capaz de ejecutar la misión críticas SAP cargas de trabajo en un scaaleable, compatible y la plataforma probada en enterprise.  Obtener escalabilidad hello, flexibilidad y los ahorros de Azure. Con la asociación de hello expandido entre Microsoft y SAP, puede ejecutar las aplicaciones de SAP en los escenarios de desarrollo/pruebas y producción de azure: y siendo totalmente compatibles. Desde SAP NetWeaver tooSAP S4/HANA, tooWindows de Linux, tooSQL de SAP HANA, tenemos que necesita. 

Con los servicios de máquinas virtuales de Microsoft Azure y SAP HANA en instancias grandes de Azure, Microsoft ofrece una completa plataforma de infraestructura como servicio (IaaS). Dada la amplia gama de soluciones SAP que se admiten en Azure, este "documento de introducción" servirá de tabla de contenido para nuestro conjunto actual de documentos de SAP. Como títulos más agréguese biblioteca de documentos de tooour - verá agregan aquí. 
...
| --- | --- | --- |
| SAP HANA Developer Edition (incluido el software de cliente HANA Hola formada por SQLODBC, solo ODBO Windows, ODBC, controladores JDBC, HANA studio y la base de datos HANA) |Red Hat Enterprise Linux, SUSE Linux Enterprise |A7, A8 |
...
| --- | --- | --- | --- |
| Software SAP Business Suite |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (solo Windows), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP Business All-in-One |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (solo Windows), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP BusinessObjects BI |Windows |N/D |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP NetWeaver |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (solo Windows), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
...
Resumen: Esta guía de inicio rápido le ayudará a tooset una sola instancia SAP HANA prototipo/demostración sistema en máquinas virtuales de Azure mediante una instalación manual de SAP NetWeaver 7.5 y SAP HANA SP12. Guía de Hola se da por supuesto que Hola lector está familiarizado con conceptos básicos de IaaS de Azure como cómo toodeploy las máquinas virtuales o redes virtuales a través de hello portal de Azure o Powershell/CLI incluidos Hola plantillas de opción toouse json. Además se espera que Hola lector está familiarizado con SAP HANA, SAP NetWeaver y cómo tooinstall de forma local.
...
Resumen: Este artículo describe cómo SAP toodeploy IDE que se ejecutan con SQL Server y el sistema operativo de Windows en Microsoft Azure a través de la biblioteca de SAP en la nube Appliance 3.0. 

Actualizado: septiembre de 2016

[Esta guía se puede encontrar aquí](sap-cal-ides-erp6-ehp7-sp3-sql.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)

## <a name="3da0389e-708b-4e82-b2a2-e92f132df89c"></a>Planeamiento e implementación
título: aaaSAP NetWeaver en máquinas virtuales de Azure (VM): Guía de implementación y planificación
...
título: aaaSAP NetWeaver en máquinas virtuales de Azure (VM): Guía de implementación
...
título: aaaSAP NetWeaver en máquinas virtuales de Azure (VM): Guía de implementación de DBMS

Resumen: En este documento se trata las consideraciones de planificación e implementación para los sistemas DBMS de Hola que deben ejecutarse junto con SAP. En la primera parte de hello, se enumeran consideraciones generales y se presentan. Hello las partes siguientes de papel Hola relacionan toodeployments de diferentes DBMS en Azure que son compatibles con SAP. Los distintos DBMS que se presentan son SQL Server, SAP ASE y Oracle. En esas partes específicas se tratan las consideraciones que tener tooaccount para cuando se ejecutan los sistemas SAP en Azure junto con los DBMS. Asuntos como los métodos de copia de seguridad y de alta disponibilidad que son compatibles con hello que distintos DBMS en Azure se presentan para el uso de hello con aplicaciones de SAP.
...
título: aaaSAP NetWeaver en máquinas virtuales de Azure (VM): Guía de implementación de alta disponibilidad

Resumen: En este documento se describe cómo se puede proteger punto único de SAP de componentes de error como Hola ASCS/SCS de SAP y DBMS en Azure. Componentes de hello SAP ASCS/SCS, DBMS y las aplicaciones son esenciales para la funcionalidad de Hola de los sistemas SAP NetWeaver, por ejemplo, ABAP de SAP NetWeaver, Java de SAP NetWeaver, sistemas de SAP NetWeaver ABAP + Java servidores. Por lo tanto, necesidades de funcionalidad de alta disponibilidad toobe colocar en coloque toomake seguro de que dichos componentes pueden soportar un error de un servidor o una máquina virtual tal como hizo con las configuraciones de clúster de Windows para entornos de Hyper-V y de reconstrucción completa.
