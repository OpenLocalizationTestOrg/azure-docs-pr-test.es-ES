...
---
# <a name="step-1-review-hello-architecture"></a>Paso 1: Revisar la arquitectura de Hola


Este artículo describen los componentes de Hola y procesos que se utilizan al replicar locales máquinas virtuales de Hyper-V en nubes de System Center Virtual Machine Manager (VMM), tooAzure con hello [Azure Site Recovery](site-recovery-overview.md) servicio.
...
Hay una serie de componentes implicados cuando se replican las máquinas virtuales de Hyper-V en tooAzure de nubes VMM.
...
**Servidor VMM** | servidor VMM Hello tiene una o más nubes que contiene hosts de Hyper-V. | En el servidor VMM Hola instalar la replicación de tooorchestrate de proveedor de Site Recovery Hola con Site Recovery y registrar el servidor de Hola Hola que del almacén de servicios de recuperación.
**Host de Hyper-V** | Uno o más hosts o clústeres de Hyper-V administrados por VMM. |  Instalar a agente de servicios de recuperación de hello en cada miembro del clúster o host.
**Máquinas virtuales de Hyper-V** | Una o varias máquinas virtuales en un servidor host de Hyper-V. | Es necesario ninguna acción tooexplicitly instalado en las máquinas virtuales.
**Redes** |Lógico y redes de VM configurado en el servidor VMM de Hola. Una red de VM debe ser tooa vinculado red lógica que esté asociada con la nube de Hola. | Redes de VM son asignadas tooAzure redes virtuales, para que las máquinas virtuales de Azure se encuentran en una red cuando se crean después de la conmutación por error.
...
**Figura 1: Replicar las máquinas virtuales en hosts de Hyper-V en tooAzure de nubes VMM**
...
4. Puede supervisar el trabajo de Hola Hola **trabajos** ficha.      ![Lista Trabajos](media/vmm-to-azure-walkthrough-architecture/image1.png)![Detalles de Habilitar protección](media/vmm-to-azure-walkthrough-architecture/image2.png)
...
1. Al finalizar la replicación inicial de hello, hello **finalizar la protección en la máquina virtual de hello** trabajo configura la red y otras configuraciones posteriores a la replicación, para que la máquina virtual de hello está protegida.
    ![Trabajo de finalización de la protección](media/vmm-to-azure-walkthrough-architecture/image3.png)
...
Vaya demasiado[paso 2: revisar los requisitos previos de implementación de Hola](vmm-to-azure-walkthrough-prerequisites.md)
