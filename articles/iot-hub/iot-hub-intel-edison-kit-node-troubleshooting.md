...
Una vez que el motor de depuración de hello está listo, debe ser capaz de toosee ```Debugger listening on port 5858``` de salida de la consola de Hola.

### <a name="configure-vs-code-tooconnect-toohello-remote-device"></a>Configurar el dispositivo remoto de VS Code tooconnect toohello

Abra hello **depurar** panel desde el lado izquierdo de Hola.

Haga clic en hello verde **Iniciar depuración** botón (F5). Normalmente se abrirían VS Code un **launch.json** de archivos, lo que necesita tooupdate.

Hola de actualización **launch.json** contenido de archivo con hello siguientes, reemplace `[device hostname or IP address]` con la dirección IP de dispositivo real de Hola o nombre de host.  
...
Haga clic en hello verde **Iniciar depuración** (F5) botón y disfrute de depuración.

Puede leer [JavaScript en VS Code](https://code.visualstudio.com/docs/languages/javascript#_debugging) toolearn más información sobre el depurador de Hola.
...
Vuelva demasiado[empezar a trabajar con Intel Edison (Node.js)](iot-hub-intel-edison-kit-node-get-started.md)
