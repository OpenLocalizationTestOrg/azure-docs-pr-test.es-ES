...
¿Aún no tiene un kit? Intente hello [frambuesa Pi 3 emulador](https://blogs.msdn.microsoft.com/iliast/2016/11/10/how-to-emulate-raspberry-pi/). También puede comprar un nuevo kit [aquí](https://azure.microsoft.com/develop/iot/starter-kits).
...
1. Descargue Raspbian.
   1. [Descargar Raspbian Jessie con píxeles](https://www.raspberrypi.org/downloads/raspbian/) (archivo .zip de hello).
...
Utilice hello breadboard y puentes cables tooconnect un LED y un tooPi BME280 como se indica a continuación. Si no dispone de sensor hello, omita esta sección.
...
1. Utilice uno de hello siguiendo a los clientes SSH de su tooyour de tooconnect del equipo host frambuesa Pi.
    - [PuTTY](http://www.putty.org/) para Windows. Necesitará Hola dirección IP de su tooconnect Pi esta a través de SSH.
    - cliente SSH integrado de Hello en Ubuntu o macOS. Debe ejecutar `ssh pi@<ip address of pi>` tooconnect Pi mediante SSH.
...
1. Instale todos los paquetes mediante el siguiente comando de Hola. Incluye el SDK de dispositivo IoT de Azure, la biblioteca del sensor BME280 y la biblioteca de cableado de Pi.
   ...
   ```
   > [!NOTE] 
   Es posible que tarde varios toofinish minutos este denpening del proceso de instalación en la conexión de red.
...
1. Abra el archivo de configuración de hello ejecutando Hola siguientes comandos:
   ...
   Hay dos elementos en este archivo que se pueden configurar. Hello primero uno es `interval`, que define el intervalo de tiempo de hello entre dos mensajes que envían toocloud. Hola otra `simulatedData`, que es un valor booleano para si toouse simulado los datos de sensor o no.
