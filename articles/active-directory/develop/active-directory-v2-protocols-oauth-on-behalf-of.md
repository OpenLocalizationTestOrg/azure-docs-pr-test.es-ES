---
title: v2.0 aaaAzure AD en nombre de flujo de OAuth2.0 | Documentos de Microsoft
...
Suponga ese usuario Hola se ha autenticado en una aplicación con hello [flujo de concesión de código de autorización de OAuth 2.0](active-directory-v2-protocols-oauth-code.md). En este momento, aplicación hello tiene un token de acceso (un símbolo (token)) con notificaciones de usuario de Hola y web de nivel intermedio de consentimiento tooaccess Hola API (API de A). Ahora, la API A debe toomake una API web de nivel inferior de toohello de solicitud autenticada (API B).
...
> [!NOTE]
> En este escenario, servicio de nivel intermedio de hello no dispone de ningún usuario interacción tooobtain Hola del usuario API de nivel inferior de hello tooaccess de consentimiento. Por lo tanto, Hola opción toogrant acceso toohello API de nivel inferior se presenta por adelantado como parte del paso de consentimiento de Hola durante la autenticación.
>

## Solicitud de token de acceso de tooservice de servicio
toorequest un token de acceso, hacer que un extremo de v2.0 de HTTP POST toohello específico del inquilino Azure AD con hello parámetros siguientes.
...
| client_id |requerido | Hola aplicación Id. que hello [Portal de registro de aplicación](https://apps.dev.microsoft.com/?referrer=https://azure.microsoft.com/documentation/articles&deeplink=/appList) asignado tooyour aplicación. |
| client_secret |requerido | secreto de aplicación Hola generadas para la aplicación Hola Portal de registro de aplicación. |
| Aserción |requerido | valor de Hola de hello token que se utiliza en la solicitud de saludo. |
| ámbito |requerido | Lista de ámbitos para solicitud de token de hello separada de un espacio. Para obtener más información, vea [Ámbitos](active-directory-v2-scopes.md).|
...
#### Ejemplo
Hola siguiente HTTP POST solicita un token de acceso con `user.read` ámbito de la API de web de https://graph.microsoft.com Hola.
...
#### Ejemplo
Hola siguiente HTTP POST solicita un token de acceso con `user.read` ámbito de hello https://graph.microsoft.com API web con un certificado.
...
Hello en el ejemplo siguiente se muestra una solicitud de tooa de respuesta correcta para un token de acceso de API de web de https://graph.microsoft.com Hola.
