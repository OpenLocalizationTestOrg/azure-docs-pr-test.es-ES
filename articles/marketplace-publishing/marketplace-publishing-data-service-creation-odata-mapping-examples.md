...
---
# <a name="examples-of-mapping-an-existing-web-service-tooodata-through-csdls"></a>Ejemplos de la asignación existente de web tooOData de servicio a través de CSDLs
> [!IMPORTANT]
...
Utilice toocreate de datos de entrada sin formato nuevo subordinado y devolver su servidor había definido por la URL(location) o tooupdate parte de hello subordinado en el servidor de hello dirección URL.  Donde hello subordinado es una secuencia, es decir, datos no estructurada, p. ej. un archivo de texto.  Tenga en cuenta que POST no es idempotentes sin una ubicación.
...
## <a name="example-functionimport-using-delete"></a>Ejemplo: FunctionImport mediante "DELETE"
Usar DELETE tooremove un URI especificado.
...
## <a name="example-functionimport-using-post"></a>Ejemplo: FunctionImport mediante "POST"
Utilice toocreate de datos de entrada sin formato nuevo subordinado y devolver su servidor había definido por la URL(location) o tooupdate parte de hello subordinado en el servidor de hello dirección URL.  Donde hello subordinado es una estructura. Tenga en cuenta que POST no es idempotentes sin una ubicación.
...
## <a name="example-functionimport-using-put"></a>Ejemplo: FunctionImport mediante "PUT"
Usar PUT toocreate subordinado nueva o subordinado completo de hello tooupdate en un servidor definido dirección URL.  Donde hello subordinado es una estructura, PUT es idempotente para varias repeticiones dará como resultado Hola mismo estado, es decir x = 5.  Put debe usarse con hello completas de contenidos de hello especificada recursos.
...
Usar toocreate colocar sin formato de datos una nueva subordinada o subordinado de tooupdate Hola todo en una dirección URL de servidor definido.  Donde hello subordinado es una secuencia, es decir, datos no estructurada, p. ej. un archivo de texto.  PUT es idempotente para varias repeticiones dará como resultado Hola mismo estado, es decir x = 5.  Put debe usarse con hello completas de contenidos de hello especificada recursos.
...
## <a name="example-functionimport-for-raw-data-returned-using-get"></a>Ejemplo: FunctionImport para los datos de "Raw" devueltos mediante "GET"
Use obtener sin procesar datos tooreturn subordinado que no está estructurado, es decir, texto.
...
## <a name="example-functionimport-for-paging-through-returned-data"></a>Ejemplo: FunctionImport para "Paging" a través de los datos devueltos
Use la paginación RESTful de implementación a través de sus datos con GET.  Paginación predeterminada se establece too100 filas por página de datos.
