---
title: "redirección de aaaConfigure para puerta de enlace de la aplicación de Azure - PowerShell | Documentos de Microsoft"
description: "Esta página permite la redirección de tooconfigure de escenarios de puerta de enlace de aplicaciones utilizando PowerShell"
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="653a6-103">Configuración del redireccionamiento en Application Gateway con PowerShell</span><span class="sxs-lookup"><span data-stu-id="653a6-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="653a6-104">Puerta de enlace de aplicaciones admite tráfico de tooredirect de capacidad de hello basándose en una configuración definida.</span><span class="sxs-lookup"><span data-stu-id="653a6-104">Application gateway supports hello ability tooredirect traffic based on a defined configuration.</span></span> <span data-ttu-id="653a6-105">más información acerca de la redirección de la visita general, toolearn [introducción de redirección de puerta de enlace de aplicaciones](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="653a6-105">toolearn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="653a6-106">Este artículo proporcionan ejemplos de la redirección de HTTP tooHTTPS, en función de la ruta de acceso redirige las redirecciones de varios sitios y redirige tooexternal sitios.</span><span class="sxs-lookup"><span data-stu-id="653a6-106">This article provides examples of HTTP tooHTTPS redirection, path based redirects, multi-site redirects, and redirects tooexternal sites.</span></span>

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="653a6-107">Redirigir HTTP tooHTTPS en una puerta de enlace de la aplicación existente</span><span class="sxs-lookup"><span data-stu-id="653a6-107">HTTP tooHTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="653a6-108">Hello en el ejemplo siguiente se crea un nuevo agente de escucha HTTP en el puerto 80 tooredirect solicitudes toohello agente de escucha existente en el puerto 443.</span><span class="sxs-lookup"><span data-stu-id="653a6-108">hello following example creates a new HTTP listener on port 80 tooredirect requests toohello existing listener on port 443.</span></span>
...
<span data-ttu-id="653a6-109">Una vez que se actualiza la puerta de enlace de aplicaciones de hello, pruebe el nuevo agente de escucha hello en el puerto 80.</span><span class="sxs-lookup"><span data-stu-id="653a6-109">Once hello application gateway is updated, test hello new listener on port 80.</span></span>  <span data-ttu-id="653a6-110">Hello en el ejemplo siguiente se muestra el uso `curl` comprobar el redireccionamiento de Hola.</span><span class="sxs-lookup"><span data-stu-id="653a6-110">hello following example shows using `curl` test hello redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="653a6-111">Redireccionamiento basado en la ruta</span><span class="sxs-lookup"><span data-stu-id="653a6-111">Path based redirect</span></span>

<span data-ttu-id="653a6-112">Hola de ejemplo siguiente crea un nuevo agente de escucha y una regla de ruta de acceso basada en una puerta de enlace de aplicación existente que redirige solo el tráfico que cumple la ruta de acceso de dirección url específica Hola.</span><span class="sxs-lookup"><span data-stu-id="653a6-112">hello following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting hello specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="653a6-113">Redireccionamiento multisitio</span><span class="sxs-lookup"><span data-stu-id="653a6-113">Multi-site redirect</span></span>

<span data-ttu-id="653a6-114">Hello en el ejemplo siguiente se crea una nueva puerta de enlace de la aplicación con 2 agentes de escucha de varios sitios en el puerto 80.</span><span class="sxs-lookup"><span data-stu-id="653a6-114">hello following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="653a6-115">los agentes de escucha de Hello son para adatum.com y adatum.org. Se crea una regla de redirección tooredirect tráfico de adatum.org tooadatum.com. Se requiere configuración adicional para configurar CNAME alias toohello aplicación puerta de enlace dirección IP pública, para obtener más información acerca de cómo delegar la tooAzure de dominio DNS y crear CNAME visitan registros de su dominio, [delegar un dominio tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="653a6-115">hello listeners are for adatum.com and adatum.org. A redirect rule is created tooredirect traffic from adatum.org tooadatum.com. Additional configuration is required for configuring CNAME aliases toohello application gateway public IP address, for more information on delegating your domain tooAzure DNS and creating CNAME records for your domain visit, [Delegate a domain tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-tooan-external-site"></a><span data-ttu-id="653a6-116">Redirigir el sitio externo tooan</span><span class="sxs-lookup"><span data-stu-id="653a6-116">Redirect tooan external site</span></span>

<span data-ttu-id="653a6-117">Hello en el ejemplo siguiente se redirige el tráfico entra en hello aplicación puerta de enlace tooan sitio Web externo (bing.com).</span><span class="sxs-lookup"><span data-stu-id="653a6-117">hello following example redirects traffic coming into hello application gateway tooan external website (bing.com).</span></span> <span data-ttu-id="653a6-118">Cuando se usa hello `TargetUrl` conmutador hello `IncludePath` y `IncludeQuery` no se permiten modificadores.</span><span class="sxs-lookup"><span data-stu-id="653a6-118">When using hello `TargetUrl` switch hello `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="653a6-119">Pasos siguientes</span><span class="sxs-lookup"><span data-stu-id="653a6-119">Next steps</span></span>

<span data-ttu-id="653a6-120">Visite [configurar final tooend SSL con la puerta de enlace de aplicaciones con PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="653a6-120">Visit [configure end tooend SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>
