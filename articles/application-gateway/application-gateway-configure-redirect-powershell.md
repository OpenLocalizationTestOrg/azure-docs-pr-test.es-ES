...
Puerta de enlace de aplicaciones admite tráfico de tooredirect de capacidad de hello basándose en una configuración definida. más información acerca de la redirección de la visita general, toolearn [introducción de redirección de puerta de enlace de aplicaciones](application-gateway-redirect-overview.md). Este artículo proporcionan ejemplos de la redirección de HTTP tooHTTPS, en función de la ruta de acceso redirige las redirecciones de varios sitios y redirige tooexternal sitios.

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a>Redirigir HTTP tooHTTPS en una puerta de enlace de la aplicación existente

Hello en el ejemplo siguiente se crea un nuevo agente de escucha HTTP en el puerto 80 tooredirect solicitudes toohello agente de escucha existente en el puerto 443.
...
Una vez que se actualiza la puerta de enlace de aplicaciones de hello, pruebe el nuevo agente de escucha hello en el puerto 80.  Hello en el ejemplo siguiente se muestra el uso `curl` comprobar el redireccionamiento de Hola.
...
Hola de ejemplo siguiente crea un nuevo agente de escucha y una regla de ruta de acceso basada en una puerta de enlace de aplicación existente que redirige solo el tráfico que cumple la ruta de acceso de dirección url específica Hola.
...
Hello en el ejemplo siguiente se crea una nueva puerta de enlace de la aplicación con 2 agentes de escucha de varios sitios en el puerto 80. los agentes de escucha de Hello son para adatum.com y adatum.org. Se crea una regla de redirección tooredirect tráfico de adatum.org tooadatum.com. Se requiere configuración adicional para configurar CNAME alias toohello aplicación puerta de enlace dirección IP pública, para obtener más información acerca de cómo delegar la tooAzure de dominio DNS y crear CNAME visitan registros de su dominio, [delegar un dominio tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).
...
## <a name="redirect-tooan-external-site"></a>Redirigir el sitio externo tooan

Hello en el ejemplo siguiente se redirige el tráfico entra en hello aplicación puerta de enlace tooan sitio Web externo (bing.com). Cuando se usa hello `TargetUrl` conmutador hello `IncludePath` y `IncludeQuery` no se permiten modificadores.
...
Visite [configurar final tooend SSL con la puerta de enlace de aplicaciones con PowerShell](application-gateway-end-to-end-ssl-powershell.md)
