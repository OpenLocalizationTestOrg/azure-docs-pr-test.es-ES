...
Obtenga información acerca de cómo tooconfigure versiones de directivas SSL y conjuntos de puerta de enlace de aplicaciones de cifrado. Puede seleccionar entre una [lista de directivas predefinidas](#predefined-ssl-policies) que contienen diferentes configuraciones de las versiones de directivas SSL y conjuntos de cifrado habilitados. También tiene Hola capacidad toodefine una [directiva personalizada de SSL](#configure-a-custom-ssl-policy) según sus requisitos.

## <a name="get-available-ssl-options"></a>Obtención de las opciones SSL disponibles

Hola `Get-AzureRMApplicationGatewayAvailableSslOptions` cmdlet proporciona una lista de directivas predefinidas disponibles, conjuntos de cifrado disponibles y las versiones de protocolo que se pueden configurar. Hello en el ejemplo siguiente se muestra un ejemplo de ejecución del cmdlet Hola salida.
...
Application Gateway incluye 3 directivas predefinidas que se pueden utilizar. Hola `Get-AzureRmApplicationGatewaySslPredefinedPolicy` cmdlet recupera estas directivas. Cada directiva tiene diferentes versiones de protocolo y conjuntos de cifrado habilitados. Se pueden usar estas directivas predefinidas tooquickly configurar una directiva de SSL en la puerta de enlace de la aplicación. De forma predeterminada se selecciona **AppGwSslPolicy20170401** si no se ha definido ninguna directiva SSL específica.

Hello aquí te mostramos un ejemplo de ejecución `Get-AzureRmApplicationGatewaySslPredefinedPolicy`.
...
Hola de ejemplo siguiente establece una directiva personalizada de SSL en una puerta de enlace de la aplicación. Establece la versión de protocolo mínimo de Hola de demasiado`TLSv1_1` y habilita Hola siguiendo conjuntos de cifrado:

* TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384
* TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256

> [!IMPORTANT]
> Conjunto de al menos un cifrado de hello lista siguiente debe seleccionarse al configurar una directiva personalizada de SSL. Application Gateway usa conjuntos de cifrado RSA SHA256 para la administración de back-end.
> * TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 
> * TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256
> * TLS_DHE_RSA_WITH_AES_128_GCM_SHA256
> * TLS_RSA_WITH_AES_128_GCM_SHA256
...
Hello en el ejemplo siguiente se crea una nueva puerta de enlace de la aplicación con una directiva predefinida de SSL.
...
Visite [introducción de redirección de puerta de enlace de aplicaciones](application-gateway-redirect-overview.md) toolearn cómo tooredirect HTTP tráfico tooa extremo HTTPS.
