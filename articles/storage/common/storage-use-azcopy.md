...
## <a name="download-and-install-azcopy"></a><span data-ttu-id="c6e0a-112">Descarga e instalación de AzCopy</span><span class="sxs-lookup"><span data-stu-id="c6e0a-112">Download and install AzCopy</span></span>
...
<span data-ttu-id="c6e0a-131">Si no especifica la opción `/S`, no se descarga ningún blob.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-131">If you do not specify option `/S`, no blobs are downloaded.</span></span>
...
<span data-ttu-id="c6e0a-147">Si Hola especifica el directorio virtual no existe, se AzCopy carga Hola archivo tooinclude Hola directorio virtual en su nombre (*p. ej.*, `vd/abc.txt` en el ejemplo de Hola anterior).</span><span class="sxs-lookup"><span data-stu-id="c6e0a-147">If hello specified virtual directory does not exist, AzCopy uploads hello file tooinclude hello virtual directory in its name (*e.g.*, `vd/abc.txt` in hello example above).</span></span>
...
<span data-ttu-id="c6e0a-212">índice de intervalo de claves de partición de Hello es 0 si el usuario de hello no especifica la opción `/PKRS`.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-212">hello partition key range index is 0 if hello user does not specify option `/PKRS`.</span></span>
...
### <a name="specify-a-shared-access-signature-sas"></a><span data-ttu-id="c6e0a-271">Especificación de una firma de acceso compartido (SAS)</span><span class="sxs-lookup"><span data-stu-id="c6e0a-271">Specify a shared access signature (SAS)</span></span>
...
<span data-ttu-id="c6e0a-289">De lo contrario, puede crear un archivo de registro en una ubicación personalizada:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-289">Otherwise, you can create an log file in a custom location:</span></span>
...
<span data-ttu-id="c6e0a-299">y tablas:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-299">and Tables:</span></span>
...
<span data-ttu-id="c6e0a-313">**Aplicable a:** blobs, archivos, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-313">**Applicable to:** Blobs, Files, Tables</span></span>
...
<span data-ttu-id="c6e0a-326">Hola patrón de archivo predeterminado que usa cuando no se especifica ningún patrón de archivo es *.*</span><span class="sxs-lookup"><span data-stu-id="c6e0a-326">hello default file pattern used when no file pattern is specified is *.*</span></span> <span data-ttu-id="c6e0a-327">para una ubicación del sistema de archivos, o un prefijo vacío para una ubicación de Almacenamiento de Azure.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-327">for a file system location or an empty prefix for an Azure Storage location.</span></span> <span data-ttu-id="c6e0a-328">No se admite la especificación de varios patrones de archivos.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-328">Specifying multiple file patterns is not supported.</span></span>
...
<span data-ttu-id="c6e0a-341">**Aplicable a:** blobs, archivos, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-341">**Applicable to:** Blobs, Files, Tables</span></span>
...
<span data-ttu-id="c6e0a-357">**Aplicable a:** blobs</span><span class="sxs-lookup"><span data-stu-id="c6e0a-357">**Applicable to:** Blobs</span></span>
...
<span data-ttu-id="c6e0a-379">Si no se especifica esta opción, o se especifica sin una ruta de acceso de carpeta, AzCopy crea el archivo de diario de hello en la ubicación predeterminada de hello, que es % LocalAppData%\Microsoft\Azure\AzCopy.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-379">If this option is not specified, or it is specified without a folder path, then AzCopy creates hello journal file in hello default location, which is %LocalAppData%\Microsoft\Azure\AzCopy.</span></span>

<span data-ttu-id="c6e0a-380">Cada vez que se emite un tooAzCopy de comando, comprueba si existe un archivo de diario en la carpeta predeterminada de hello, o si se encuentra en una carpeta que especifica mediante esta opción.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-380">Each time you issue a command tooAzCopy, it checks whether a journal file exists in hello default folder, or whether it exists in a folder that you specified via this option.</span></span> <span data-ttu-id="c6e0a-381">Si no existe el archivo de diario de Hola en cualquier lugar, AzCopy trata operación hello como nueva y genera un nuevo archivo de diario.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-381">If hello journal file does not exist in either place, AzCopy treats hello operation as new and generates a new journal file.</span></span>

<span data-ttu-id="c6e0a-382">Si existe el archivo de diario de hello, AzCopy comprueba si línea de comandos de hello especificado coincide con hello de línea de comandos en el archivo de diario de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-382">If hello journal file does exist, AzCopy checks whether hello command line that you input matches hello command line in hello journal file.</span></span> <span data-ttu-id="c6e0a-383">Si coincide con dos líneas de comandos hello, AzCopy reanuda la operación incompleta de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-383">If hello two command lines match, AzCopy resumes hello incomplete operation.</span></span> <span data-ttu-id="c6e0a-384">Si no coinciden, son tooeither solicitadas sobrescribir Hola diario archivo toostart una operación de nuevo o toocancel Hola operación actual.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-384">If they do not match, you are prompted tooeither overwrite hello journal file toostart a new operation, or toocancel hello current operation.</span></span>

<span data-ttu-id="c6e0a-385">archivo de diario de Hola se elimina tras la finalización correcta de operación de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-385">hello journal file is deleted upon successful completion of hello operation.</span></span>

<span data-ttu-id="c6e0a-386">Tenga en cuenta que la reanudación de una operación a partir de un archivo de diario creado por una versión anterior de AzCopy no se admite.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-386">Note that resuming an operation from a journal file created by a previous version of AzCopy is not supported.</span></span>

<span data-ttu-id="c6e0a-387">**Aplicable a:** blobs, archivos, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-387">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="parameter-file"></a><span data-ttu-id="c6e0a-388">/@:"parameter-file"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-388">/@:"parameter-file"</span></span>
<span data-ttu-id="c6e0a-389">Especifica un archivo que contiene parámetros.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-389">Specifies a file that contains parameters.</span></span> <span data-ttu-id="c6e0a-390">Procesos de AzCopy Hola parámetros en el archivo hello como si se hubieran especificado en la línea de comandos de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-390">AzCopy processes hello parameters in hello file just as if they had been specified on hello command line.</span></span>

<span data-ttu-id="c6e0a-391">En un archivo de respuesta, puede especificar varios parámetros en una sola línea o especificar cada parámetro en su propia línea.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-391">In a response file, you can either specify multiple parameters on a single line, or specify each parameter on its own line.</span></span> <span data-ttu-id="c6e0a-392">Tenga en cuenta que un parámetro individual no puede ocupar varias líneas.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-392">Note that an individual parameter cannot span multiple lines.</span></span>

<span data-ttu-id="c6e0a-393">Archivos de respuesta pueden incluir líneas de comentarios que comienzan por el símbolo # de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-393">Response files can include comments lines that begin with hello # symbol.</span></span>

<span data-ttu-id="c6e0a-394">Puede especificar varios archivos de respuesta.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-394">You can specify multiple response files.</span></span> <span data-ttu-id="c6e0a-395">Sin embargo, tenga en cuenta que AzCopy no admite archivos de respuesta anidados.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-395">However, note that AzCopy does not support nested response files.</span></span>

<span data-ttu-id="c6e0a-396">**Aplicable a:** blobs, archivos, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-396">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="y"></a><span data-ttu-id="c6e0a-397">/Y</span><span class="sxs-lookup"><span data-stu-id="c6e0a-397">/Y</span></span>
<span data-ttu-id="c6e0a-398">Suprime todos los mensajes de confirmación de AzCopy.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-398">Suppresses all AzCopy confirmation prompts.</span></span>

<span data-ttu-id="c6e0a-399">**Aplicable a:** blobs, archivos, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-399">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="l"></a><span data-ttu-id="c6e0a-400">/L</span><span class="sxs-lookup"><span data-stu-id="c6e0a-400">/L</span></span>
<span data-ttu-id="c6e0a-401">Especifica solamente una operación de descripción; no se copian datos.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-401">Specifies a listing operation only; no data is copied.</span></span>

<span data-ttu-id="c6e0a-402">AzCopy interpreta Hola uso de esta opción como una simulación de línea de comandos de ejecución Hola sin esta opción /L y recuentos de cuántos objetos se copian, puede especificar opción /V al mismo tiempo toocheck qué objetos se copian en el registro detallado de Hola de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-402">AzCopy interprets hello using of this option as a simulation for running hello command line without this option /L and counts how many objects are copied, you can specify option /V at hello same time toocheck which objects are copied in hello verbose log.</span></span>

<span data-ttu-id="c6e0a-403">comportamiento de Hola de esta opción también viene determinado por la ubicación de Hola Hola de datos de origen y la presencia de Hola de hello recursiva modo /S y archivo patrón opción /Pattern.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-403">hello behavior of this option is also determined by hello location of hello source data and hello presence of hello recursive mode option /S and file pattern option /Pattern.</span></span>

<span data-ttu-id="c6e0a-404">AzCopy requiere el permiso LISTA y LECTURA de esta ubicación de origen cuando se usa esta opción.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-404">AzCopy requires LIST and READ permission of this source location when using this option.</span></span>

<span data-ttu-id="c6e0a-405">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-405">**Applicable to:** Blobs, Files</span></span>

### <a name="mt"></a><span data-ttu-id="c6e0a-406">/MT</span><span class="sxs-lookup"><span data-stu-id="c6e0a-406">/MT</span></span>
<span data-ttu-id="c6e0a-407">Establece toobe Hola igual como blob de origen de Hola o del archivo de la hora de la última modificación del archivo descargado Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-407">Sets hello downloaded file's last-modified time toobe hello same as hello source blob or file's.</span></span>

<span data-ttu-id="c6e0a-408">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-408">**Applicable to:** Blobs, Files</span></span>

### <a name="xn"></a><span data-ttu-id="c6e0a-409">/XN</span><span class="sxs-lookup"><span data-stu-id="c6e0a-409">/XN</span></span>
<span data-ttu-id="c6e0a-410">Excluye un recurso origen más nuevo.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-410">Excludes a newer source resource.</span></span> <span data-ttu-id="c6e0a-411">Hello recursos no si se copian hora de última modificación del origen de Hola de hello es hello igual o más reciente que el destino.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-411">hello resource is not copied if hello last modified time of hello source is hello same or newer than destination.</span></span>

<span data-ttu-id="c6e0a-412">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-412">**Applicable to:** Blobs, Files</span></span>

### <a name="xo"></a><span data-ttu-id="c6e0a-413">/XO</span><span class="sxs-lookup"><span data-stu-id="c6e0a-413">/XO</span></span>
<span data-ttu-id="c6e0a-414">Excluye un recurso de origen más antiguo.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-414">Excludes an older source resource.</span></span> <span data-ttu-id="c6e0a-415">Hello recursos no si se copian hora de última modificación del origen de Hola de hello es hello igual o anterior a destino.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-415">hello resource is not copied if hello last modified time of hello source is hello same or older than destination.</span></span>

<span data-ttu-id="c6e0a-416">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-416">**Applicable to:** Blobs, Files</span></span>

### <a name="a"></a><span data-ttu-id="c6e0a-417">/A</span><span class="sxs-lookup"><span data-stu-id="c6e0a-417">/A</span></span>
<span data-ttu-id="c6e0a-418">Carga sólo los archivos que tienen el atributo Archive de hello establecido.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-418">Uploads only files that have hello Archive attribute set.</span></span>

<span data-ttu-id="c6e0a-419">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-419">**Applicable to:** Blobs, Files</span></span>

### <a name="iarashcnetoi"></a><span data-ttu-id="c6e0a-420">/IA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="c6e0a-420">/IA:[RASHCNETOI]</span></span>
<span data-ttu-id="c6e0a-421">Solo los archivos de carga que contenga cualquiera de Hola especifican el conjunto de atributos.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-421">Uploads only files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="c6e0a-422">Los atributos disponibles son los siguientes:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-422">Available attributes include:</span></span>

* <span data-ttu-id="c6e0a-423">R = Archivos de solo lectura</span><span class="sxs-lookup"><span data-stu-id="c6e0a-423">R = Read-only files</span></span>
* <span data-ttu-id="c6e0a-424">A = Archivos listos para archivarse</span><span class="sxs-lookup"><span data-stu-id="c6e0a-424">A = Files ready for archiving</span></span>
* <span data-ttu-id="c6e0a-425">S = Archivos del sistema</span><span class="sxs-lookup"><span data-stu-id="c6e0a-425">S = System files</span></span>
* <span data-ttu-id="c6e0a-426">H = Archivos ocultos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-426">H = Hidden files</span></span>
* <span data-ttu-id="c6e0a-427">C = Archivos comprimidos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-427">C = Compressed files</span></span>
* <span data-ttu-id="c6e0a-428">N = Archivos normales</span><span class="sxs-lookup"><span data-stu-id="c6e0a-428">N = Normal files</span></span>
* <span data-ttu-id="c6e0a-429">E = Archivos cifrados</span><span class="sxs-lookup"><span data-stu-id="c6e0a-429">E = Encrypted files</span></span>
* <span data-ttu-id="c6e0a-430">T = Archivos temporales</span><span class="sxs-lookup"><span data-stu-id="c6e0a-430">T = Temporary files</span></span>
* <span data-ttu-id="c6e0a-431">O = Archivos fuera de línea</span><span class="sxs-lookup"><span data-stu-id="c6e0a-431">O = Offline files</span></span>
* <span data-ttu-id="c6e0a-432">I = Archivos no indexados</span><span class="sxs-lookup"><span data-stu-id="c6e0a-432">I = Non-indexed files</span></span>

<span data-ttu-id="c6e0a-433">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-433">**Applicable to:** Blobs, Files</span></span>

### <a name="xarashcnetoi"></a><span data-ttu-id="c6e0a-434">/XA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="c6e0a-434">/XA:[RASHCNETOI]</span></span>
<span data-ttu-id="c6e0a-435">Excluye los archivos que contengan cualquiera de hello especificado establecido algún atributo.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-435">Excludes files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="c6e0a-436">Los atributos disponibles son los siguientes:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-436">Available attributes include:</span></span>

* <span data-ttu-id="c6e0a-437">R = Archivos de solo lectura</span><span class="sxs-lookup"><span data-stu-id="c6e0a-437">R = Read-only files</span></span>
* <span data-ttu-id="c6e0a-438">A = Archivos listos para archivarse</span><span class="sxs-lookup"><span data-stu-id="c6e0a-438">A = Files ready for archiving</span></span>
* <span data-ttu-id="c6e0a-439">S = Archivos del sistema</span><span class="sxs-lookup"><span data-stu-id="c6e0a-439">S = System files</span></span>
* <span data-ttu-id="c6e0a-440">H = Archivos ocultos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-440">H = Hidden files</span></span>
* <span data-ttu-id="c6e0a-441">C = Archivos comprimidos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-441">C = Compressed files</span></span>
* <span data-ttu-id="c6e0a-442">N = Archivos normales</span><span class="sxs-lookup"><span data-stu-id="c6e0a-442">N = Normal files</span></span>
* <span data-ttu-id="c6e0a-443">E = Archivos cifrados</span><span class="sxs-lookup"><span data-stu-id="c6e0a-443">E = Encrypted files</span></span>
* <span data-ttu-id="c6e0a-444">T = Archivos temporales</span><span class="sxs-lookup"><span data-stu-id="c6e0a-444">T = Temporary files</span></span>
* <span data-ttu-id="c6e0a-445">O = Archivos fuera de línea</span><span class="sxs-lookup"><span data-stu-id="c6e0a-445">O = Offline files</span></span>
* <span data-ttu-id="c6e0a-446">I = Archivos no indexados</span><span class="sxs-lookup"><span data-stu-id="c6e0a-446">I = Non-indexed files</span></span>

<span data-ttu-id="c6e0a-447">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-447">**Applicable to:** Blobs, Files</span></span>

### <a name="delimiterdelimiter"></a><span data-ttu-id="c6e0a-448">/Delimiter:"delimiter"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-448">/Delimiter:"delimiter"</span></span>
<span data-ttu-id="c6e0a-449">Indica el carácter de delimitador de hello usa toodelimit los directorios virtuales en un nombre de blob.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-449">Indicates hello delimiter character used toodelimit virtual directories in a blob name.</span></span>

<span data-ttu-id="c6e0a-450">De forma predeterminada, AzCopy utiliza / como carácter de delimitador de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-450">By default, AzCopy uses / as hello delimiter character.</span></span> <span data-ttu-id="c6e0a-451">Sin embargo, AzCopy admite el uso de cualquier carácter convencional (como @, # o %) como delimitador.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-451">However, AzCopy supports using any common character (such as @, #, or %) as a delimiter.</span></span> <span data-ttu-id="c6e0a-452">Si necesita tooinclude uno de estos caracteres especiales en la línea de comandos de hello, ponga el nombre de archivo de hello entre comillas dobles.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-452">If you need tooinclude one of these special characters on hello command line, enclose hello file name with double quotes.</span></span>

<span data-ttu-id="c6e0a-453">Esta opción solamente se aplica para descarga de blobs.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-453">This option is only applicable for downloading blobs.</span></span>

<span data-ttu-id="c6e0a-454">**Aplicable a:** blobs</span><span class="sxs-lookup"><span data-stu-id="c6e0a-454">**Applicable to:** Blobs</span></span>

### <a name="ncnumber-of-concurrent-operations"></a><span data-ttu-id="c6e0a-455">/NC:"number-of-concurrent-operations"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-455">/NC:"number-of-concurrent-operations"</span></span>
<span data-ttu-id="c6e0a-456">Especifica el número de Hola de operaciones simultáneas.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-456">Specifies hello number of concurrent operations.</span></span>

<span data-ttu-id="c6e0a-457">AzCopy forma predeterminada, inicia un número determinado de rendimiento de transferencia de datos de las operaciones simultáneas tooincrease Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-457">AzCopy by default starts a certain number of concurrent operations tooincrease hello data transfer throughput.</span></span> <span data-ttu-id="c6e0a-458">Tenga en cuenta que gran número de operaciones simultáneas en un entorno de ancho de banda bajo puede saturar la conexión de red de Hola y evitar las operaciones de hello complete totalmente.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-458">Note that large number of concurrent operations in a low-bandwidth environment may overwhelm hello network connection and prevent hello operations from fully completing.</span></span> <span data-ttu-id="c6e0a-459">Reduzca el número de operaciones simultáneas basándose en el ancho de banda de red disponible real.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-459">Throttle concurrent operations based on actual available network bandwidth.</span></span>

<span data-ttu-id="c6e0a-460">límite superior de Hola para las operaciones simultáneas es 512.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-460">hello upper limit for concurrent operations is 512.</span></span>

<span data-ttu-id="c6e0a-461">**Aplicable a:** blobs, archivos, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-461">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="sourcetypeblob--table"></a><span data-ttu-id="c6e0a-462">/SourceType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-462">/SourceType:"Blob" | "Table"</span></span>
<span data-ttu-id="c6e0a-463">Especifica que hello `source` recurso es un blob disponible en el entorno de desarrollo local hello, ejecute en un emulador de almacenamiento de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-463">Specifies that hello `source` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="c6e0a-464">**Aplicable a:** blobs, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-464">**Applicable to:** Blobs, Tables</span></span>

### <a name="desttypeblob--table"></a><span data-ttu-id="c6e0a-465">/DestType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-465">/DestType:"Blob" | "Table"</span></span>
<span data-ttu-id="c6e0a-466">Especifica que hello `destination` recurso es un blob disponible en el entorno de desarrollo local hello, ejecute en un emulador de almacenamiento de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-466">Specifies that hello `destination` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="c6e0a-467">**Aplicable a:** blobs, tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-467">**Applicable to:** Blobs, Tables</span></span>

### <a name="pkrskey1key2key3"></a><span data-ttu-id="c6e0a-468">/PKRS:"key1#key2#key3#..."</span><span class="sxs-lookup"><span data-stu-id="c6e0a-468">/PKRS:"key1#key2#key3#..."</span></span>
<span data-ttu-id="c6e0a-469">Divisiones Hola tooenable de intervalo de claves de partición exportar datos de la tabla en paralelo, lo que aumenta la velocidad de Hola de operación de exportación de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-469">Splits hello partition key range tooenable exporting table data in parallel, which increases hello speed of hello export operation.</span></span>

<span data-ttu-id="c6e0a-470">Si no se especifica esta opción, AzCopy utiliza un entidades de tabla de tooexport de subproceso único.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-470">If this option is not specified, then AzCopy uses a single thread tooexport table entities.</span></span> <span data-ttu-id="c6e0a-471">Por ejemplo, si hello usuario especifica /PKRS: "aa #bb" y, a continuación, AzCopy inicia tres operaciones simultáneas.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-471">For example, if hello user specifies /PKRS:"aa#bb", then AzCopy starts three concurrent operations.</span></span>

<span data-ttu-id="c6e0a-472">Cada operación exporta uno de los tres rangos de claves de partición, como se muestra a continuación:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-472">Each operation exports one of three partition key ranges, as shown below:</span></span>

  <span data-ttu-id="c6e0a-473">[first-partition-key, aa)</span><span class="sxs-lookup"><span data-stu-id="c6e0a-473">[first-partition-key, aa)</span></span>

  <span data-ttu-id="c6e0a-474">[aa, bb)</span><span class="sxs-lookup"><span data-stu-id="c6e0a-474">[aa, bb)</span></span>

  <span data-ttu-id="c6e0a-475">[bb, last-partition-key]</span><span class="sxs-lookup"><span data-stu-id="c6e0a-475">[bb, last-partition-key]</span></span>

<span data-ttu-id="c6e0a-476">**Aplicable a:** tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-476">**Applicable to:** Tables</span></span>

### <a name="splitsizefile-size"></a><span data-ttu-id="c6e0a-477">/SplitSize:"file-size"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-477">/SplitSize:"file-size"</span></span>
<span data-ttu-id="c6e0a-478">Especifica Hola archivo exportado dividir tamaño en MB, Hola valor mínimo permitido es de 32.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-478">Specifies hello exported file split size in MB, hello minimal value allowed is 32.</span></span>

<span data-ttu-id="c6e0a-479">Si no se especifica esta opción, AzCopy exporta tooa de datos de tabla de archivos únicos.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-479">If this option is not specified, AzCopy exports table data tooa single file.</span></span>

<span data-ttu-id="c6e0a-480">Si datos de la tabla de hello están blob tooa exportado y tamaño del archivo exportado hello alcanza límite de 200 GB de hello para el tamaño del blob, AzCopy divide Hola archivo exportado, incluso si no se especifica esta opción.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-480">If hello table data is exported tooa blob, and hello exported file size reaches hello 200 GB limit for blob size, then AzCopy splits hello exported file, even if this option is not specified.</span></span>

<span data-ttu-id="c6e0a-481">**Aplicable a:** tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-481">**Applicable to:** Tables</span></span>

### <a name="entityoperationinsertorskip--insertormerge--insertorreplace"></a><span data-ttu-id="c6e0a-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span></span>
<span data-ttu-id="c6e0a-483">Especifica el comportamiento de importación de datos de tabla de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-483">Specifies hello table data import behavior.</span></span>

* <span data-ttu-id="c6e0a-484">InsertOrSkip - omite una entidad existente o inserta una nueva entidad si no existe en la tabla de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-484">InsertOrSkip - Skips an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="c6e0a-485">InsertOrMerge - combina una entidad existente o inserta una nueva entidad si no existe en la tabla de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-485">InsertOrMerge - Merges an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="c6e0a-486">InsertOrReplace - reemplaza una entidad existente o inserta una nueva entidad si no existe en la tabla de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-486">InsertOrReplace - Replaces an existing entity or inserts a new entity if it does not exist in hello table.</span></span>

<span data-ttu-id="c6e0a-487">**Aplicable a:** tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-487">**Applicable to:** Tables</span></span>

### <a name="manifestmanifest-file"></a><span data-ttu-id="c6e0a-488">/Manifest:"manifest-file"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-488">/Manifest:"manifest-file"</span></span>
<span data-ttu-id="c6e0a-489">Especifica el archivo de manifiesto de Hola para tabla Hola exportación e importación de operación.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-489">Specifies hello manifest file for hello table export and import operation.</span></span>

<span data-ttu-id="c6e0a-490">Esta opción es opcional durante la operación de exportación de hello, AzCopy genera un archivo de manifiesto con nombre predefinido, si no se especifica esta opción.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-490">This option is optional during hello export operation, AzCopy generates a manifest file with predefined name if this option is not specified.</span></span>

<span data-ttu-id="c6e0a-491">Esta opción es necesaria durante la operación de importación de Hola para buscar archivos de datos de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-491">This option is required during hello import operation for locating hello data files.</span></span>

<span data-ttu-id="c6e0a-492">**Aplicable a:** tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-492">**Applicable to:** Tables</span></span>

### <a name="synccopy"></a><span data-ttu-id="c6e0a-493">/SyncCopy</span><span class="sxs-lookup"><span data-stu-id="c6e0a-493">/SyncCopy</span></span>
<span data-ttu-id="c6e0a-494">Indica si toosynchronously copiar blobs o archivos entre dos extremos de almacenamiento de Azure.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-494">Indicates whether toosynchronously copy blobs or files between two Azure Storage endpoints.</span></span>

<span data-ttu-id="c6e0a-495">De forma predeterminada, AzCopy usa la copia asincrónica del servidor.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-495">AzCopy by default uses server-side asynchronous copy.</span></span> <span data-ttu-id="c6e0a-496">Especifique este tooperform opción sincrónica copy, que descarga de blobs o archivos toolocal memoria y, a continuación, carga tooAzure almacenamiento.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-496">Specify this option tooperform a synchronous copy, which downloads blobs or files toolocal memory and then uploads them tooAzure Storage.</span></span>

<span data-ttu-id="c6e0a-497">Puede usar esta opción cuando se copian archivos dentro del almacenamiento de blobs, almacenamiento de archivos, o desde Blob almacenamiento tooFile almacenamiento o viceversa.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-497">You can use this option when copying files within Blob storage, within File storage, or from Blob storage tooFile storage or vice versa.</span></span>

<span data-ttu-id="c6e0a-498">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-498">**Applicable to:** Blobs, Files</span></span>

### <a name="setcontenttypecontent-type"></a><span data-ttu-id="c6e0a-499">/SetContentType:"content-type"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-499">/SetContentType:"content-type"</span></span>
<span data-ttu-id="c6e0a-500">Especifica el tipo de contenido MIME de Hola de blobs de destino o los archivos.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-500">Specifies hello MIME content type for destination blobs or files.</span></span>

<span data-ttu-id="c6e0a-501">Conjuntos de AzCopy Hola de tipo de contenido para un blob o tooapplication/octet-stream de archivos de forma predeterminada.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-501">AzCopy sets hello content type for a blob or file tooapplication/octet-stream by default.</span></span> <span data-ttu-id="c6e0a-502">Puede establecer el tipo de contenido de Hola para todos los blobs o archivos especificando explícitamente un valor para esta opción.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-502">You can set hello content type for all blobs or files by explicitly specifying a value for this option.</span></span>

<span data-ttu-id="c6e0a-503">Si se especifica esta opción sin ningún valor, AzCopy establece cada blob o tipo de contenido del archivo según tooits extensión de archivo.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-503">If you specify this option without a value, then AzCopy sets each blob or file's content type according tooits file extension.</span></span>

<span data-ttu-id="c6e0a-504">**Aplicable a:** blobs, archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-504">**Applicable to:** Blobs, Files</span></span>

### <a name="payloadformatjson--csv"></a><span data-ttu-id="c6e0a-505">/PayloadFormat:"JSON" | "CSV"</span><span class="sxs-lookup"><span data-stu-id="c6e0a-505">/PayloadFormat:"JSON" | "CSV"</span></span>
<span data-ttu-id="c6e0a-506">Especifica el formato de Hola Hola datos exportados del archivo de tabla.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-506">Specifies hello format of hello table exported data file.</span></span>

<span data-ttu-id="c6e0a-507">Si no se especifica esta opción, de forma predeterminada, AzCopy exporta el archivo de datos de tabla en formato JSON.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-507">If this option is not specified, by default AzCopy exports table data file in JSON format.</span></span>

<span data-ttu-id="c6e0a-508">**Aplicable a:** tablas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-508">**Applicable to:** Tables</span></span>

## <a name="known-issues-and-best-practices"></a><span data-ttu-id="c6e0a-509">Problemas conocidos y prácticas recomendadas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-509">Known Issues and Best Practices</span></span>
### <a name="limit-concurrent-writes-while-copying-data"></a><span data-ttu-id="c6e0a-510">Limitación de escrituras concurrentes mientras se copian datos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-510">Limit concurrent writes while copying data</span></span>
<span data-ttu-id="c6e0a-511">Cuando copia blobs o archivos con AzCopy, tenga en cuenta que otra aplicación puede modificar datos de hello mientras que va a copiar.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-511">When you copy blobs or files with AzCopy, keep in mind that another application may be modifying hello data while you are copying it.</span></span> <span data-ttu-id="c6e0a-512">Si es posible, asegúrese de que los datos de hello que va a copiar no se está modificando durante la operación de copia de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-512">If possible, ensure that hello data you are copying is not being modified during hello copy operation.</span></span> <span data-ttu-id="c6e0a-513">Por ejemplo, al copiar un disco duro virtual asociado a una máquina virtual de Azure, asegúrese de que ninguna otra aplicación actualmente está escribiendo toohello disco duro virtual.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-513">For example, when copying a VHD associated with an Azure virtual machine, make sure that no other applications are currently writing toohello VHD.</span></span> <span data-ttu-id="c6e0a-514">Una buena manera toodo es mediante la concesión de hello recursos toobe copiado.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-514">A good way toodo this is by leasing hello resource toobe copied.</span></span> <span data-ttu-id="c6e0a-515">Como alternativa, puede crear una instantánea de disco duro virtual de hello en primer lugar y, a continuación, copiar instantáneas Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-515">Alternately, you can create a snapshot of hello VHD first and then copy hello snapshot.</span></span>

<span data-ttu-id="c6e0a-516">Si no se puede impedir que otras aplicaciones escribir tooblobs o archivos mientras se se copian y tenga en cuenta que al trabajo Hola Hola finaliza, hello recursos copiados pueden ya no tiene paridad completa con recursos de origen de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-516">If you cannot prevent other applications from writing tooblobs or files while they are being copied, then keep in mind that by hello time hello job finishes, hello copied resources may no longer have full parity with hello source resources.</span></span>

### <a name="run-one-azcopy-instance-on-one-machine"></a><span data-ttu-id="c6e0a-517">Ejecutar una instancia de AzCopy en un solo equipo.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-517">Run one AzCopy instance on one machine.</span></span>
<span data-ttu-id="c6e0a-518">AzCopy es toomaximize diseñada Hola utilización de la transferencia de datos de máquina recursos tooaccelerate hello, le recomendamos que ejecute solo una instancia de AzCopy en un equipo y especifica la opción de hello `/NC` si necesita las operaciones más simultáneas.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-518">AzCopy is designed toomaximize hello utilization of your machine resource tooaccelerate hello data transfer, we recommend you run only one AzCopy instance on one machine, and specify hello option `/NC` if you need more concurrent operations.</span></span> <span data-ttu-id="c6e0a-519">Para obtener más información, escriba `AzCopy /?:NC` en línea de comandos de Hola.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-519">For more details, type `AzCopy /?:NC` at hello command line.</span></span>

### <a name="enable-fips-compliant-md5-algorithms-for-azcopy-when-you-use-fips-compliant-algorithms-for-encryption-hashing-and-signing"></a><span data-ttu-id="c6e0a-520">Habilitar algoritmos MD5 que cumplan con FIPS para AzCopy cuando "Use algoritmos que cumplen con FIPS para el cifrado, firma y operaciones hash".</span><span class="sxs-lookup"><span data-stu-id="c6e0a-520">Enable FIPS compliant MD5 algorithms for AzCopy when you "Use FIPS compliant algorithms for encryption, hashing and signing".</span></span>
<span data-ttu-id="c6e0a-521">AzCopy predeterminada usa hello toocalculate de implementación .NET MD5 MD5 al copiar los objetos, pero hay algunos requisitos de seguridad que necesitan AzCopy tooenable FIPS MD5 comprobar si es compatible.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-521">AzCopy by default uses .NET MD5 implementation toocalculate hello MD5 when copying objects, but there are some security requirements that need AzCopy tooenable FIPS compliant MD5 setting.</span></span>

<span data-ttu-id="c6e0a-522">Puede crear un archivo app.config `AzCopy.exe.config` con la propiedad `AzureStorageUseV1MD5` y separarlo con AzCopy.exe.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-522">You can create an app.config file `AzCopy.exe.config` with property `AzureStorageUseV1MD5` and put it aside with AzCopy.exe.</span></span>
...
<span data-ttu-id="c6e0a-523">Para la propiedad "AzureStorageUseV1MD5" • True: valor predeterminado es hello AzCopy utiliza implementación MD5. NET.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-523">For property "AzureStorageUseV1MD5" • True - hello default value, AzCopy uses .NET MD5 implementation.</span></span>
<span data-ttu-id="c6e0a-524">• False: AzCopy utiliza el algoritmo MD5 compatible con FIPS.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-524">• False – AzCopy uses FIPS compliant MD5 algorithm.</span></span>

<span data-ttu-id="c6e0a-525">Tenga en cuenta que los algoritmos compatibles con FIPS están deshabilitados de forma predeterminada en el equipo de Windows, puede escribir secpol.msc en la ventana de ejecución y comprobar este modificador en Configuración de seguridad -> Directivas locales -> Opciones de seguridad -> Criptografía de sistema: Usar algoritmos compatibles con FIPS para cifrado, firma y operaciones hash.</span><span class="sxs-lookup"><span data-stu-id="c6e0a-525">Note that FIPS compliant algorithms is disabled by default on your Windows machine, you can type secpol.msc in your Run window and check this switch at Security Setting->Local Policy->Security Options->System cryptography: Use FIPS compliant algorithms for encryption, hashing and signing.</span></span>

## <a name="next-steps"></a><span data-ttu-id="c6e0a-526">Pasos siguientes</span><span class="sxs-lookup"><span data-stu-id="c6e0a-526">Next steps</span></span>
<span data-ttu-id="c6e0a-527">Para obtener más información sobre el almacenamiento de Azure y AzCopy, vea Hola recursos siguientes:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-527">For more information about Azure Storage and AzCopy, see hello following resources:</span></span>

### <a name="azure-storage-documentation"></a><span data-ttu-id="c6e0a-528">Documentación de Almacenamiento de Azure:</span><span class="sxs-lookup"><span data-stu-id="c6e0a-528">Azure Storage documentation:</span></span>
* [<span data-ttu-id="c6e0a-529">Introducción tooAzure almacenamiento</span><span class="sxs-lookup"><span data-stu-id="c6e0a-529">Introduction tooAzure Storage</span></span>](../storage-introduction.md)
* [<span data-ttu-id="c6e0a-530">¿Cómo toouse almacenamiento de blobs en .NET</span><span class="sxs-lookup"><span data-stu-id="c6e0a-530">How toouse Blob storage from .NET</span></span>](../blobs/storage-dotnet-how-to-use-blobs.md)
* [<span data-ttu-id="c6e0a-531">¿Cómo toouse almacenamiento de archivos de .NET</span><span class="sxs-lookup"><span data-stu-id="c6e0a-531">How toouse File storage from .NET</span></span>](../storage-dotnet-how-to-use-files.md)
* [<span data-ttu-id="c6e0a-532">¿Cómo toouse almacenamiento de tablas de .NET</span><span class="sxs-lookup"><span data-stu-id="c6e0a-532">How toouse Table storage from .NET</span></span>](../../cosmos-db/table-storage-how-to-use-dotnet.md)
* [<span data-ttu-id="c6e0a-533">Cómo administrar toocreate, o eliminar una cuenta de almacenamiento</span><span class="sxs-lookup"><span data-stu-id="c6e0a-533">How toocreate, manage, or delete a storage account</span></span>](../storage-create-storage-account.md)
* [<span data-ttu-id="c6e0a-534">Transferencia de datos con AzCopy en Linux</span><span class="sxs-lookup"><span data-stu-id="c6e0a-534">Transfer data with AzCopy on Linux</span></span>](storage-use-azcopy-linux.md)

### <a name="azure-storage-blog-posts"></a><span data-ttu-id="c6e0a-535">Publicaciones en blobs de Almacenamiento de Azure</span><span class="sxs-lookup"><span data-stu-id="c6e0a-535">Azure Storage blog posts:</span></span>
* [<span data-ttu-id="c6e0a-536">Introducción a la versión de vista previa de la biblioteca de movimiento de datos de Almacenamiento de Azure</span><span class="sxs-lookup"><span data-stu-id="c6e0a-536">Introducing Azure Storage Data Movement Library Preview</span></span>](https://azure.microsoft.com/blog/introducing-azure-storage-data-movement-library-preview-2/)
* [<span data-ttu-id="c6e0a-537">AzCopy: Introducción a la copia sincrónica y el tipo de contenido personalizado</span><span class="sxs-lookup"><span data-stu-id="c6e0a-537">AzCopy: Introducing synchronous copy and customized content type</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2015/01/13/azcopy-introducing-synchronous-copy-and-customized-content-type.aspx)
* [<span data-ttu-id="c6e0a-538">AzCopy: Presentación de la disponibilidad general de AzCopy 3.0 y de la versión de vista previa de AzCopy 4.0 compatible con tablas y archivos</span><span class="sxs-lookup"><span data-stu-id="c6e0a-538">AzCopy: Announcing General Availability of AzCopy 3.0 plus preview release of AzCopy 4.0 with Table and File support</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/10/29/azcopy-announcing-general-availability-of-azcopy-3-0-plus-preview-release-of-azcopy-4-0-with-table-and-file-support.aspx)
* [<span data-ttu-id="c6e0a-539">AzCopy: Optimizada para escenarios de copia a gran escala</span><span class="sxs-lookup"><span data-stu-id="c6e0a-539">AzCopy: Optimized for Large-Scale Copy Scenarios</span></span>](http://go.microsoft.com/fwlink/?LinkId=507682)
* [<span data-ttu-id="c6e0a-540">AzCopy: Compatibilidad para almacenamiento con redundancia geográfica de acceso de lectura</span><span class="sxs-lookup"><span data-stu-id="c6e0a-540">AzCopy: Support for read-access geo-redundant storage</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/04/07/azcopy-support-for-read-access-geo-redundant-account.aspx)
* [<span data-ttu-id="c6e0a-541">AzCopy: Transferencia de datos con modo reiniciable y token SAS</span><span class="sxs-lookup"><span data-stu-id="c6e0a-541">AzCopy: Transfer data with re-startable mode and SAS token</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/09/07/azcopy-transfer-data-with-re-startable-mode-and-sas-token.aspx)
* [<span data-ttu-id="c6e0a-542">AzCopy: Uso de copia de blobs entre cuentas</span><span class="sxs-lookup"><span data-stu-id="c6e0a-542">AzCopy: Using cross-account Copy Blob</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/04/01/azcopy-using-cross-account-copy-blob.aspx)
* [<span data-ttu-id="c6e0a-543">AzCopy: Carga y descarga de archivos para blobs de Azure</span><span class="sxs-lookup"><span data-stu-id="c6e0a-543">AzCopy: Uploading/downloading files for Azure Blobs</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2012/12/03/azcopy-uploading-downloading-files-for-windows-azure-blobs.aspx)
